                                ...
                                <div class="card-body card-dashboard">
                                    <x-tables.basic-table>
                                        <x-slot name="header">
                                            ...
                                            {{-- <th class="th-global-class  text-center">{{__('Can Receive Travel Info')}}</th> --}}
                                            ...
                                            <tr data-id="{{ $model->id }}" class="deleteable-row">
                                                ...
                                                {{-- <td class="text-center">
                                                    ...
                                                    <div class="form-group pb-1">
                                                        <x-form.checkbox-element data-toggle-route="{{ $toggleCanReceiveTravelInfos }}" data-id="{{ $model->id }}" class="switch-trigger-js" :is-required="false" :name="'can_receive_travel_info'" :is-checked="$model->getCanReceiveTravelInfo()"> </x-form.checkbox-element>
                                                    </div>
                                                    @else
                                                    {{$model->getCanReceiveTravelInfo() ? __('Active') : __('Non-Active')}}
                                                    @endif
                                                </td> --}}
                                                @if($user->can(getPermissionName('update')) || $user->can(getPermissionName('delete')) )
