            ...
            <div class="content-body">
                <section id="file-export">
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                @if(admin()->check_route_permission('admin.reasons.index') == 1)
                                <div class="card-header" style="padding-bottom: 0px">
                                    <h4 class="card-title">{{__('msg.reasons')}}</h4>
                                    <a class="heading-elements-toggle"><i
                                            class="la la-ellipsis-v font-medium-3"></i></a>
                                    <div class="heading-elements">
                                        <ul class="list-inline mb-0">
                                            @if(admin()->check_route_permission('admin.reasons.create') == 1)
                                                <li>
                                                    <a href="{{route('admin.reasons.create')}}" style="border-radius: 10px;padding: 10px"
                                    ...
                                    <div class="card-body card-dashboard">
                                            <table class="table table-striped table-bordered file-export">
                                                ...
                                                @foreach($reasons as $reason)
                                                    <tr class="row_{{$reason->id}}">
                                                        <td class="text-center">
                                                            {{$reason->id}}
                                                        </td>
                                                        <td class="text-center">{{$reason->title}}</td>
                                                        <td class="d-flex align-items-center justify-content-sm-center">
                                                            @if(admin()->check_route_permission('admin.reasons.delete') == 1)
                                                                <button type="button" item_id="{{$reason->id}}" class="delete_item btn btn-danger btn-sm">
                                                                    <i class="la la-trash"></i></button>
                                                            @endif
                                                                @if(admin()->check_route_permission('admin.reasons.edit') == 1)
...
<script>
    $(document).on('click','.delete_item',function (){
        ...
        $(document).on('click','.confirm_delete',function (){
            $.ajax({
                type : 'post',
                url: "{{route('admin.reasons.delete')}}",
                data:{
                    '_token' : "{{csrf_token()}}",
                    'id' : item_id
                },
