                                ...
                                <div class="card-body card-dashboard">
                                    <x-tables.basic-table>
                                        <x-slot name="header">
                                            ...
                                            <th class="th-global-class  text-center">{{__('Description')}}</th>
											<th class="th-global-class  text-center">{{__('Section')}}</th>
                                        ...
                                        <x-slot name="body">
                                            ...
                                            <tr data-id="{{ $model->id }}" class="deleteable-row">
                                                ...
                                                <td class="text-center">{{$model->getDescription($lang)}}</td>
												 <td class="text-center">{{$model->getSectionNameFormatted()}}</td>
                                                <td class="text-center">
                                                    @if($user->can(getPermissionName('update')))
                                                    <div class="form-group pb-1">
                                                        <x-form.checkbox-element data-toggle-route="{{ $toggleIsActiveRoute }}" data-id="{{ $model->id }}" class="switch-trigger-js" :is-required="false" :name="'is_active'" :is-checked="$model->getIsActive()"> </x-form.checkbox-element>
                                                    </div>
                                                    @else
                                                    {{$model->getIsActive() ? __('Active') : __('Non-Active')}}
                                                    @endif
                                                </td>
                                                @if($user->can(getPermissionName('update')) || $user->can(getPermissionName('delete')) )
