...
<body class="">
	<div class="container">
		...
		<h1 class="spacer main-header"> {{ $websiteName }}</h1>
		<p class=" description"> {{ $textMessage }} </p>
		...
		<p class="spacer description">{{ __('if you have any question, just email us at') }} <a href="mailto:{{ $email }}" style="color:#5680BD !important;">{{ $email }}</a> - {{ __('we are alawys happy to help out') }}  </p>
		<p class="description">{{ __('Regards') }}, <br> {{ __('Team') }} {{ $websiteName }}  </p>
	</div>
</body>
</html>
